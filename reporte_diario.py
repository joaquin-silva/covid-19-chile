...
def data_criticos():
    df = pd.read_csv(
        "https://raw.githubusercontent.com/MinCiencia/Datos-COVID19/master/output/producto23/PacientesCriticos_std.csv"
    )
...
def data_nacionales():
    ...
    df["nuevos fallecidos"] = [
        df["Fallecidos"][i] - df["Fallecidos"][i - 1] if df["Fallecidos"][i] > 0 else 0
...
def my_plot(df, col, referencia, tipo):
    ...
    fig.add_trace(
        go.Scatter(x=df["Fecha reporte"], y=df[col], marker_color="cadetblue")
    )
...
def my_plot_2(df, col):
    ...
    fig = go.Figure(
        go.Scatter(x=df["Fecha reporte"], y=df[col], marker_color="cadetblue", name=col)
    )
...
def get_data_reg():
    ...
    df = df.query(
        'Categoria == "Casos nuevos totales" and Region != "Total"'
...
def main():
    ...
    referencia = st.sidebar.selectbox(
        ...
        ["Día anterior", "7 días atrás", "14 días atrás", "21 días atrás"],
    )
    ...
    if op:
        ...
        columns = ["Casos nuevos totales", "Nuevos fallecidos", "Positividad"]
