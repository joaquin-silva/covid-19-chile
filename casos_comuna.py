...
def get_data():
    df = pd.read_csv(
        "https://raw.githubusercontent.com/MinCiencia/Datos-COVID19/master/output/producto45/CasosConfirmadosPorComuna_std.csv"
    )
    df["Año"] = [int(str(semana)[:4]) for semana in df["Semana Epidemiologica"]]
    df["Numero Semana"] = [
        int(str(semana)[-2:]) for semana in df["Semana Epidemiologica"]
    ]
...
def my_heatmap(df, comunas, op, op_data):
    ...
    fig = go.Figure(
        data=go.Heatmap(
            z=z, x=data["Numero Semana"], y=data["Comuna"], colorscale="inferno_r"
        )
    )
...
def main():
    ...
    op_data = st.sidebar.selectbox(
        "Datos",
        ["Casos confirmados", "Casos nuevos por fecha de inicio de síntomas"],
        key=0,
    )
    ...
    select = st.multiselect(
        "Seleccionar comunas", comunas, ["Antofagasta", "Puente Alto", "Punta Arenas"]
    )
    ...
    st.markdown(
        "**Nota:** Los datos de las últimas semanas se pueden ir ajustando con el paso del tiempo."
    )
