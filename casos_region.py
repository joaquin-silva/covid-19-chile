...
def my_percentage_plot(df, value_name, title):
    ...
    chart = (
        ...
        .properties(title=title, height=600, width=600)
    )
    return chart
...
def main():
    ...
    if start_date > end_date:
        st.sidebar.error(
            "Error: La fecha de término debe ser después de la fecha de inicio."
        )

    opciones = [
        "Total de casos confirmados acumulados",
        "Total de casos confirmados acumulados por 100.000 habitantes",
        "Nuevos casos confirmados",
        "Nuevos casos confirmados por 100.000 habitantes",
    ]
    plot = st.selectbox("Elegir gráfico", opciones)
    ...
    if plot == "Nuevos casos confirmados":
        ...
        pop = get_population()
        df = df.T
        pop = pop.reset_index().append(
            {"Region": "Total", "Poblacion": pop["Poblacion"].sum()}, ignore_index=True
        )
    ...
    if plot == "Total de casos confirmados acumulados":
        ...
        st.title(
            "Evolución del total de casos confirmados acumulados por 100.000 habitantes/tasa de incidencia acumulada por región"
        )
    elif plot == "Nuevos casos confirmados":
        st.title("Evolución de nuevos casos confirmados por región")
    else:
        st.title(
            "Evolución de nuevos casos confirmados por 100.000 habitantes/tasa de incidencia por región"
        )
    ...
    st.markdown(
        " - El 17 de junio, se añadieron 31.422 casos confirmados debido a revisiones en el sistema de epivigilia y las fuentes de datos ([ver noticia](https://www.biobiochile.cl/noticias/nacional/chile/2020/06/16/minsal-anade-otros-31-412-contagios-covid-19-no-estaban-informados-total-supera-los-215-mil.shtml))."
    )

    if (
        plot == "Total de casos confirmados acumulados"
        or plot == "Nuevos casos confirmados"
    ):
        st.markdown(
            "*Advertencia*: El número de casos confirmados no representa exactamente el número de casos/contagios reales. Estos últimos valores no son conocidos por límites de testeo."
        )
    else:
        st.markdown(
            "*Advertencia*: El número de casos confirmados no representa exactamente el número de casos/contagios reales. Estos últimos valores no son conocidos por límites de testeo. Además, los casos confirmados por 100.000 habitantes, dan mayores valores a regiones con pocos habitantes, por lo que pueden introducir sesgos en los datos."
        )
