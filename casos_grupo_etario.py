...
def get_data():
    df = pd.read_csv(
        "https://raw.githubusercontent.com/MinCiencia/Datos-COVID19/master/output/producto16/CasosGeneroEtario_std.csv"
    )
    data = df.groupby(["Fecha", "Grupo de edad"], as_index=False).sum()
    data = data.sort_values(["Fecha", "Grupo de edad"], ascending=True).reset_index(
        drop=True
    )
    ...
    for i in range(data.shape[0]):
        if i < 17:
            ...
            nuevos.append(
                data["Casos confirmados"][i] - data["Casos confirmados"][i - 17]
            )
...
def get_column_ine(df):
    data = pd.read_csv(
        "https://raw.githubusercontent.com/joaquin-silva/covid-19-chile/master/data/datos_ine_proyecciones.csv",
        sep=";",
    )
    data = data[["Edad", "2020"]]
...
def main():
    ...
    start = st.sidebar.date_input("Fecha de inicio", value=df["Fecha"].loc[17])
        ...
        "Fecha de término", value=df["Fecha"].loc[df.shape[0] - 1]
    )
