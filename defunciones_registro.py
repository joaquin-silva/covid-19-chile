...
def get_data():
    df = pd.read_csv(
        "https://raw.githubusercontent.com/MinCiencia/Datos-COVID19/master/output/producto32/Defunciones_std.csv"
    )
    ...
    df = df[[int(df["Año"][i]) >= 2016 for i in range(df.shape[0])]].reset_index(
        drop=True
    )
    l_semana = [
        datetime.datetime.strptime(df["Fecha"][i], "%Y-%m-%d").date().isocalendar()[1]
...
def grafico_acumulado(dfs, region):
    ...
    fig.update_layout(title=title, template="ggplot2", xaxis_title="Año", height=450)
    return fig
...
def main():
    ...
    st.header("Datos")
    st.write(
        "Cantidad de defunciones inscritas en el Registro Civil por número de semana."
    )
    df = get_data()
    ...
    reg = st.selectbox(
        "Region", regiones, index=regiones.index("Metropolitana de Santiago")
    )
