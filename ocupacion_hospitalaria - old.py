...
def get_data():
    df = pd.read_csv(
        "https://raw.githubusercontent.com/MinCiencia/Datos-COVID19/master/output/producto24/CamasHospital_Diario_T.csv"
    )
...
def get_data_sochimi():
    df = pd.read_csv(
        "https://raw.githubusercontent.com/MinCiencia/Datos-COVID19/master/output/producto48/SOCHIMI_std.csv"
    )
    df["Vmi otro"] = (
        df["Vmi ocupados"]
        - df["Vmi covid19 confirmados"]
        - df["Vmi covid19 sospechosos"]
    )
...
def my_plot_vmi(df):
    ...
    fig.add_trace(
        go.Scatter(
            x=df["Fecha"],
            y=df["Vmi covid19 confirmados"],
            name="VMI Covid-19 confirmado",
            stackgroup="one",
            marker_color="red",
        )
    )
    fig.add_trace(
        go.Scatter(
            x=df["Fecha"],
            y=df["Vmi covid19 sospechosos"],
            name="VMI Covid-19 sospechoso",
            stackgroup="one",
            marker_color="orange",
        )
    )
...
def main():
    ...
    st.write(
        "Datos de la encuesta diaria realidad nacional medicina intensiva provistos por el Ministerio de Ciencia en su [producto 48](https://github.com/MinCiencia/Datos-COVID19/tree/master/output/producto48)."
    )
    ...
    reg = st.sidebar.selectbox(
        "Elegir Región", regiones, key=0, index=regiones.index("Metropolitana")
    )
